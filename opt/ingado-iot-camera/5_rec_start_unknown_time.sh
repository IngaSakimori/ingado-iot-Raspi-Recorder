...
echo "$now_time date is unknown!!" >> $LOG_DIR/rec_script.log

#NLLモードのフラグをチェックし、有効ならばNLLSETを書き込む
if [ $NLLFLAG -eq 1 ]
then
  now_time=`date "+%Y%m%d-%H%M%S"`
  echo "$now_time(fake time) $HOSTNAME NLL mode is active. Background convert and USB memory move is stop while recording(raspivid running)" >> $LOG_DIR/rec_script.log
  NLLSET="_NLL_mode"
else
  now_time=`date "+%Y%m%d-%H%M%S"`
  echo "$now_time(fake time) $HOSTNAME Normal mode is active." >> $LOG_DIR/rec_script.log
fi
...
if [[ -f /media/pi/RASPI/stop_rec ]]; then
  ...
  (echo -e "$HOSTNAME fake-time-$now_time stop_rec file found. Rec will not start!! $MAILINFO" | mail -s "$HOSTNAME stop_rec file found. Rec will not start!! fake-time-$now_time" $MAIL_ADD)&
...
then
  ...
  (echo -e "$HOSTNAME $now_time NLL USB Memory Direct rec started!! $MAILINFO" | mail -s "$HOSTNAME NLL USB Memory Direct REC STARTED $now_time" $MAIL_ADD)&
   raspivid -o $USB_MEM_DIR/rec-start-fake-time-$now_time$NLLSET.h264 -fps $FPS -w $WIDE -h $HEIGHT $OPTION $ROTATION $BIT_RATE_FLAG $BIT_RATE -t 99999999999999999 -n $RASPIVID_VERBOSE >> $LOG_DIR/rec_script.log 2>&1
fi
...
then
  ...
  now_time=`date "+%Y%m%d-%H%M%S"`
  echo "$now_time Date is not correct. fake time $now_time rec started!!" >> $LOG_DIR/rec_script.log
  raspivid -o $SCRIPT_DIR/rec_h264/rec-start-fake-time-$now_time$NLLSET.h264 -fps $FPS -w $WIDE -h $HEIGHT $OPTION $ROTATION $BIT_RATE_FLAG $BIT_RATE -t $REC_TIME $PREVIEW $RASPIVID_VERBOSE  >> $LOG_DIR/rec_script.log 2>&1
#録画したh264は処理フォルダへ移動する
  mv $SCRIPT_DIR/rec_h264/rec-start-fake-time-$now_time$NLLSET.h264 $SCRIPT_DIR/rec_convert
