...
if [ ! "$?" -eq 0 ]
then
  echo "camera unit is not detected!! check camera unit and cable!!" | mail -s "camera unit or cable fail $now_time" $MAIL_ADD
  exit 1
else
...
#録画開始
  ...
  (echo "$now_time rec started!!" | mail -s "REC STARTED $now_time" $MAIL_ADD)&
  raspivid -o $SCRIPT_DIR/rec_h264/rec-start-$now_time.h264 -fps $FPS -w $WIDE -h $HEIGHT -b $BIT_RATE -t $REC_TIME $PREVIEW
#録画したh264は処理フォルダへ移動する
  mv $SCRIPT_DIR/rec_h264/rec-start-$now_time.h264 $SCRIPT_DIR/rec_convert
...
echo "$now_time raspi DISK_PER_Full. Stopped Rec " | mail -s " $now_time raspi DISK_PER_Full. Stopped Rec " $MAIL_ADD
    break
  fi
